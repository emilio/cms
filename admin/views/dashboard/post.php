...
<form action="<?php echo Url::get('admin@post') ?>" class="edit-form" method="POST">
	...
	<p class="span10 offset1 post-content-wrapper">
		<label for="content" class="assistive-text">Contenido del post</label>
		...
		<a id="media" class="icon icon-image icon-add-media" href="#" title="Añadir archivos multimedia"></a>
...
<script type="text/html" id="media-modal-template">
	<div class="media-gallery media-gallery-modal">
		<a href="<?php echo Url::get('admin@media_upload') ?>" class="btn btn-info" target="#blank">Subir nuevo archivo</a>
		<ul>
			{{#each item}}
				<li class="media-item">
					<img src="<?php echo Url::get('uploads'); ?>{{route}}" alt="{{title}}" class="media-item-thumbnail">
				</li>
			{{/each}}
		</ul>	
	</div>
