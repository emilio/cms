...
class Admin_Controller {
	...
	public static function action_media() {
		$media = Media::get();
		return View::make('dashboard.media')->add_var('media', $media);
	}
	public static function action_media_upload() {
		if( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
			// Upload things and show the result
		} else {
			return View::make('dashboard.media-upload');
		}
	}
	public static function action_ajax_media() {
		$media = Media::query()->limit(10)->get();
		return Response::json(array(
			'items' => $media
		));
	}
